...
# Сначала не понимал, почему выдавались ошибки, на сайте ВК работало, а при выполнениии домашки ничего не работало
# Потом нашел в документации, посмотрел ошибки, выяснил. что нужно было создать токен с бОльшими правами
SUPER_TOKEN = 'токен преподавателя с правами на просмотр друзей'
        ...
        if not isinstance(other, VkUser):
            ...
            if res['response']:
                ...
                for user in res['response']:
                    # генерируем имя переменной вида id7777777 и создаем сам новый экземпляр класса
...
# ИСПЫТАНИЯ
id6851258 = VkUser(SUPER_TOKEN, '5.126', 6851258)           # создаем экземпляры пользователей ВК с общими друзьями
id219059 = VkUser(SUPER_TOKEN, '5.126', 219059)

id6851258 & id219059                                        # ищем общих друзей, должны быть 4

id1 = VkUser(SUPER_TOKEN, '5.126', 1)                       # создаем аккаунт Павла Дурова, с которым нет общих друзей

id6851258 & id1                                             # ищем общих друзей с Павлом Дуровым

print(id1)                                                  # печатаем ссылку на страницу Павла Дурова
